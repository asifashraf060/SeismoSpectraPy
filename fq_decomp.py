...
for i in np.arange(3, len(dir_DataFolder)):
    ...
    dm_fq = [] 
    errs  = []
    for j in np.arange(18, len(df)):
        
        # specific eq info
        eq_no,  eq_name, tr_no = df['no'][j], df['name'][j], df['traces'][j]
        ...
        if len(mseed_path)>0:
            ...
            for k in np.arange(0, len(mseed_path)): 
                ...
                plt.plot(tr.data)
                plt.title('Trace ' + str(k))
            plt.subplot(1, len(mseed_path)+1, k+2)
            plt.plot(np.arange(1, len(mseed_path)+1), np.zeros(len(mseed_path))+2, '+r')
            plt.ylim([0, 3])
            plt.title('Choose the trace no')
            ...
            plt.plot(t[data.argmax()], fq_pts.max()-2, 'vc')
            for kk in fq_pts:
    ...
    with open(str(folder_path) + '/Analysis/EQ_fq_decomp_result.txt' , 'w') as file:            
        file.write(f"{'no':<15}{'name':<15}{'lat':<15}{'lon':<15}{'depth':<15}{'mag':<15}{'dist':<15}{'traces':<15}{'fq':<15}{'err':<15}\n")
        for eq_nums, eq_names, eq_lats, eq_lons, eq_depths, eq_mags, eq_dists, eq_traces, eq_fq, eq_err in zip(no, name, lat, lon, dp, mg, dist, trc, dm_fq, errs):
            file.write(f"{eq_nums:<15}{eq_names:<15}{eq_lats:<15}{eq_lons:<15}{eq_depths:<15}{eq_mags:<15}{eq_dists:<15}{eq_traces:<15}{eq_fq:<15}{eq_err:<15}\n")
